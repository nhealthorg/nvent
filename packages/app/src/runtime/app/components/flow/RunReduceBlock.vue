<template>
  <div class="run-reduce-block">
    <div class="run-reduce-block__header">
      ...
      <span :class="['run-reduce-block__status', statusClass]">{{
        statusLabel
      }}</span>
    </div>
    ...
    <div class="run-reduce-block__stats">
      <span>Results</span>
      <strong
        >store {{ step.reduceResultStoreCount || 0 }} · memory
        {{ step.reduceResultMemoryCount || 0 }} · pruned
        {{ step.reduceResultPrunedCount || 0 }}</strong
      >
    </div>
    <div
      v-if="
        step.reduceActiveIteration !== null &&
        step.reduceActiveIteration !== undefined
      "
      class="run-reduce-block__stats"
    >
      <span>Active iteration</span>
      <strong>#{{ step.reduceActiveIteration }}</strong>
    </div>
    ...
    <div class="run-reduce-block__body">
      ...
      <button
        ...
        type="button"
        :class="[
          'run-reduce-block__child',
          { 'run-reduce-block__child--selected': modelValue === item.value },
        ]"
      ...
      >
        ...
        />
        <span class="truncate">{{
          item.label || getStepDisplayName(item.step)
        }}</span>
        <span
          class="ml-auto text-[10px]"
          :class="getStepStatusTextColor(item.step.status)"
          >{{ item.step.status || "pending" }}</span
        >
      </button>
      <div v-if="!step.bodyItems?.length" class="run-reduce-block__empty">
        No reduce body steps
      </div>
    </div>
  </div>
</template>
...
const statusLabel = computed(() =>
  String(props.step?.status || "pending").toUpperCase(),
);
const statusClass = computed(() =>
  props.step?.status === "failed"
    ? "run-reduce-block__status--error"
    : props.step?.status === "completed"
      ? "run-reduce-block__status--done"
      : "",
);
</script>
...
.run-reduce-block__header,
.run-reduce-block__meta,
.run-reduce-block__progress {
  display: flex;
  align-items: center;
  justify-content: space-between;
  gap: 10px;
}
.run-reduce-block__eyebrow {
  font-size: 10px;
  letter-spacing: 0.08em;
  opacity: 0.7;
}
.run-reduce-block__title {
  margin-top: 2px;
  font-size: 13px;
  font-weight: 700;
}
.run-reduce-block__status {
  padding: 3px 7px;
  border-radius: 999px;
...
.run-reduce-block__meta,
.run-reduce-block__progress,
.run-reduce-block__stats {
  margin-top: 7px;
  padding-top: 6px;
  border-top: 1px solid rgb(194 65 12 / 0.18);
  font-size: 10px;
}
.run-reduce-block__stats strong {
  max-width: 190px;
  overflow: hidden;
...
.run-reduce-block__child:hover,
.run-reduce-block__child--selected {
  border-color: rgb(249 115 22);
  background: rgb(255 237 213);
}
.run-reduce-block__empty {
  font-size: 10px;
  font-style: italic;
  opacity: 0.65;
}
:global(.dark) .run-reduce-block {
  background: rgb(67 20 7 / 0.4);
  color: rgb(254 215 170);
}
:global(.dark) .run-reduce-block__body,
:global(.dark) .run-reduce-block__child {
