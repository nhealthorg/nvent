...
export function generateIiiConfigYaml(cfg: IiiEngineConfig): string {
  ...
  if (cfg.modules.pubsub) {
    ...
    workers.push({ name: 'pubsub', config: Object.keys(pubsubCfg).length ? pubsubCfg : undefined })
  }
  ...
  if (workerManager) {
    ...
    const execLine = [command, ...args].map(shellQuote).join(' ')
    workers.push({
      name: 'iii-exec',
      config: {
        exec: [execLine],
...
export function buildEngineConfig(
  ...
  const normalizedQueueConfigs: UserQueueConfigs = queueModuleEnabled
    ? {
        default: {
          type: 'standard',
          concurrency: 1,
